...
@test "network: /etc/resolv.conf inside RHEL 8.10" {
  run --keep-empty-lines --separate-stderr "$TOOLBOX" run --distro rhel --release 8.10 readlink /etc/resolv.conf
...
@test "network: ping(8) inside RHEL 8.10" {
  run --keep-empty-lines --separate-stderr "$TOOLBOX" run --distro rhel --release 8.10 ping -c 2 f.root-servers.net
