...
@test "cdi: Smoke test inside non-native container" {
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 true
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" \
    --distro fedora \
    --release 44 \
...
@test "cdi: create-symlinks with no link inside non-native container" {
...
@test "cdi: create-symlinks with one link (absolute target, different parent) inside non-native container" {
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 test -e /run/toolbox.1
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 readlink /run/toolbox.1
...
@test "cdi: create-symlinks with one link (absolute target, different parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with one link (absolute target, missing parent) inside non-native container" {
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 test -e /opt/bin/toolbox
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 readlink /opt/bin/toolbox
...
@test "cdi: create-symlinks with one link (absolute target, missing parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with one link (absolute target, same parent) inside non-native container" {
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 test -e /usr/bin/toolbox.1
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run --arch "$(get_cross_arch)" --distro fedora --release 44 readlink /usr/bin/toolbox.1
...
@test "cdi: create-symlinks with one link (absolute target, same parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with three links (absolute targets, mixed parents) inside non-native container" {
...
@test "cdi: create-symlinks with three links (absolute targets, mixed parents, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with one link (relative target, different parent) inside non-native container" {
...
@test "cdi: create-symlinks with one link (relative target, different parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with one link (relative target, missing parent) inside non-native container" {
...
@test "cdi: create-symlinks with one link (relative target, missing parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with one link (relative target, same parent) inside non-native container" {
...
@test "cdi: create-symlinks with one link (relative target, same parent, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: create-symlinks with three links (relative targets, mixed parents) inside non-native container" {
...
@test "cdi: create-symlinks with three links (relative targets, mixed parents, restart) inside non-native container" {
  local container_name
  container_name="fedora-toolbox-44-$(get_cross_arch)"
...
@test "cdi: ldconfig(8) with no folder inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run \
    --arch "$(get_cross_arch)" --distro fedora --release 44 test "$toolbx_runtime_directory/cdi-nvidia.json" -ot /etc/ld.so.cache
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run \
    --arch "$(get_cross_arch)" --distro fedora --release 44 test -e /etc/ld.so.conf.d/toolbx-nvidia.conf
...
@test "cdi: ldconfig(8) with one folder inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
  ...
  run --keep-empty-lines --separate-stderr "$TOOLBX" run \
    --arch "$(get_cross_arch)" --distro fedora --release 44 cat /etc/ld.so.conf.d/toolbx-nvidia.conf
...
@test "cdi: ldconfig(8) with two folders inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try invalid JSON inside non-native container" {
...
@test "cdi: Try an empty file inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try create-symlinks with invalid path inside non-native container" {
...
@test "cdi: Try create-symlinks with unknown path inside non-native container" {
...
@test "cdi: Try create-symlinks with missing --link argument inside non-native container" {
...
@test "cdi: Try create-symlinks with relative link in --link argument inside non-native container" {
...
@test "cdi: Try create-symlinks with wrongly formatted --link argument ('foo') inside non-native container" {
...
@test "cdi: Try create-symlinks with wrongly formatted --link argument ('foo::bar::baz') inside non-native container" {
...
@test "cdi: Try create-symlinks with invalid name inside non-native container" {
...
@test "cdi: Try create-symlinks with unknown name inside non-native container" {
...
@test "cdi: Try hook with invalid path inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try hook with unknown path inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try hook with unknown args inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try hook with invalid name inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try hook with unknown name inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try mount with invalid container path inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try mount with invalid host path inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
...
@test "cdi: Try mount with non-existent paths inside non-native container" {
  local toolbx_runtime_directory="$XDG_RUNTIME_DIR/toolbox"
