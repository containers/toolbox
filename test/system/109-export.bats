...
@test "export: Export GIMP app from Fedora container" {
...
@test "export: Export VLC app from Fedora container" {
  create_default_container
  install_test_apps

  run "$TOOLBX" export --app vlc --container "$(get_latest_container_name)"
  assert_success

  assert [ -f "$HOME/.local/share/applications/vlc-$(get_latest_container_name).desktop" ]
}

@test "export: Export Neovim binary from Fedora container" {
  create_default_container
  install_test_apps
